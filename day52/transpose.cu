...
__global__ void transpose_naive_kernel(float *in, float *out, size_t N,
                                       size_t M) {
...
int main() {
  ...
  cudaMalloc(&out_base_d, N * M * sizeof(float));
  ...
  transpose_naive_kernel<<<numBlocks, numThreads>>>(in_d, out_base_d, N, M);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
  CUDA_CHECK(cudaMemcpy(out_base, out_base_d, N * M * sizeof(float),
