...
__host__ __device__ float sigmoid(float x) { return 1 / (1 + expf(-x)); }

__global__ void silu_kernel(float *A, float *B, uint32_t N) {
  uint32_t i = blockDim.x * blockIdx.x + threadIdx.x;
  if (4 * i + 3 >= N)
    return;
  float4 in, out;
  in = ((float4 *)A)[i];
  out.x = in.x * sigmoid(in.x);
  out.y = in.y * sigmoid(in.y);
  out.z = in.z * sigmoid(in.z);
  out.w = in.w * sigmoid(in.w);
  ((float4 *)B)[i] = out;
}

void silu_gpu(float *A, float *B, uint32_t N) {
  float *A_d, *B_d;
  ...
  int32_t numBlocks = cdiv(N, numThreads * 4);
  start_timer(&t);
  silu_kernel<<<numBlocks, numThreads>>>(A_d, B_d, N);
...
void silu_cpu(float *A, float *B, uint32_t N) {
  start_timer(&t);
