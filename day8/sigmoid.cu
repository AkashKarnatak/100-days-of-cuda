...
__global__ void sigmoid_kernel(float *A, float *B, uint32_t N) {
...
void sigmoid_gpu(float *A, float *B, uint32_t N) {
  float *A_d, *B_d;
  ...
  start_timer(&t);
  sigmoid_kernel<<<numBlocks, numThreads>>>(A_d, B_d, N);
...
void sigmoid_cpu(float *A, float *B, uint32_t N) {
  start_timer(&t);
