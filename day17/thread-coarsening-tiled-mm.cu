...
__global__ void coarsed_tiled_matmul_kernel(float *A, float *B, float *C,
                                            uint32_t M, uint32_t N,
                                            uint32_t P) {
  uint32_t row = blockDim.y * blockIdx.y + threadIdx.y;
  uint32_t col = blockDim.x * COARSE_FACTOR * blockIdx.x + threadIdx.x;
  ...
  for (uint32_t block = 0; block < cdiv(N, BLOCK_DIM); ++block) {
    ...
    for (size_t c = 0; c < COARSE_FACTOR; ++c) {
      if (block * BLOCK_DIM + threadIdx.y < N && c * BLOCK_DIM + col < P)
        B_s[threadIdx.y][threadIdx.x] =
            B[(block * BLOCK_DIM + threadIdx.y) * P + c * BLOCK_DIM + col];
      else
  ...
  for (size_t c = 0; c < COARSE_FACTOR; ++c) {
    if (row < M && c * BLOCK_DIM + col < P)
      C[row * P + c * BLOCK_DIM + col] = sum[c];
  }
}
...
void coarsed_tiled_matmul_gpu(float *A, float *B, float *C, uint32_t M,
                              uint32_t N, uint32_t P) {
  float *A_d, *B_d, *C_d;
  ...
  dim3 numBlocks(cdiv(P, numThreads.x * COARSE_FACTOR), cdiv(M, numThreads.y));
  start_timer(&t);
  coarsed_tiled_matmul_kernel<<<numBlocks, numThreads>>>(A_d, B_d, C_d, M, N,
                                                         P);
  cudaDeviceSynchronize();
  stop_timer(&t);
  printf("GPU time(coarsed tiled): %f\n", time_diff(&t));
...
int main() {
  ...
  float *A, *B, *C_cpu, *C_gpu, *C_gpu_tiled, *C_gpu_coarsed;
  ...
  C_gpu_coarsed = (float *)malloc((M * P) * sizeof(float));
  ...
  coarsed_tiled_matmul_gpu(A, B, C_gpu_coarsed, M, N, P);
  ...
  printf("GPU coarsed tiled impl match: %s\n",
         all_close(C_cpu, C_gpu_coarsed, M, P) ? "true" : "false");
