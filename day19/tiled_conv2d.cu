...
__global__ void tiled_conv2d_kernel(float *in_data, float *out_data, size_t N,
                                    size_t M) {
  int32_t inRow = (blockDim.y - 2 * MASK_RADIUS) * blockIdx.y + threadIdx.y - MASK_RADIUS;
  int32_t inCol = (blockDim.x - 2 * MASK_RADIUS) * blockIdx.x + threadIdx.x - MASK_RADIUS;

  __shared__ float in_data_s[TILE_DIM][TILE_DIM];

  if (inRow >= 0 && inRow < (int32_t)N && inCol >= 0 && inCol < (int32_t)M) {
    in_data_s[threadIdx.y][threadIdx.x] = in_data[inRow * M + inCol];
  } else {
    in_data_s[threadIdx.y][threadIdx.x] = 0;
  }

  __syncthreads();

  if (threadIdx.x >= TILE_DIM - 2 * MASK_RADIUS ||
      threadIdx.y >= TILE_DIM - 2 * MASK_RADIUS)
    return;

  int32_t outRow = inRow + MASK_RADIUS;
  int32_t outCol = inCol + MASK_RADIUS;
  ...
  for (size_t maskRow = 0; maskRow < MASK_DIM; ++maskRow) {
    for (size_t maskCol = 0; maskCol < MASK_DIM; ++maskCol) {
      sum += mask_c[maskRow][maskCol] *
             in_data_s[threadIdx.y + maskRow][threadIdx.x + maskCol];
    }
  }
...
__global__ void conv2d_kernel(float *in_data, float *out_data, size_t N,
  ...
  size_t outCol = blockDim.x * blockIdx.x + threadIdx.x;
  if (outRow >= N || outCol >= M)
    return;
  ...
  for (size_t maskRow = 0; maskRow < MASK_DIM; ++maskRow) {
    for (size_t maskCol = 0; maskCol < MASK_DIM; ++maskCol) {
      ...
      if (inRow >= 0 && inRow < (int32_t)N && inCol >= 0 &&
          inCol < (int32_t)M) {
        sum += in_data[inRow * M + inCol] * mask_c[maskRow][maskCol];
      }
    }
  }
  out_data[outRow * M + outCol] = sum;
...
void tiled_conv2d_gpu(float *in_data, float *out_data, float *mask, size_t N,
                      size_t M) {
  ...
  dim3 numBlocks(cdiv(M, numThreads.x - MASK_RADIUS * 2),
                 cdiv(N, numThreads.y - MASK_RADIUS * 2));
  start_timer(&t);
  tiled_conv2d_kernel<<<numBlocks, numThreads>>>(in_data_d, out_data_d, N, M);
      ...
      for (size_t maskRow = 0; maskRow < MASK_DIM; ++maskRow) {
        for (size_t maskCol = 0; maskCol < MASK_DIM; ++maskCol) {
          ...
          if (inRow >= 0 && inRow < (int32_t)N && inCol >= 0 &&
              inCol < (int32_t)M) {
            sum +=
                in_data[inRow * M + inCol] * mask[maskRow * MASK_DIM + maskCol];
          }
        }
      }
      out_data[outRow * M + outCol] = sum;
...
int main() {
  ...
  float *in_data, *out_data_cpu, *out_data_gpu, *out_data_gpu_tiled, *mask;
  size_t N, M;
  ...
  out_data_gpu_tiled = (float *)malloc(N * M * sizeof(float));
  ...
  tiled_conv2d_gpu(in_data, out_data_gpu_tiled, mask, N, M);

  printf("GPU and CPU impl match: %s\n",
         allclose(out_data_cpu, out_data_gpu, N, M) ? "true" : "false");

  printf("GPU and CPU impl match: %s\n",
         allclose(out_data_cpu, out_data_gpu_tiled, N, M) ? "true" : "false");
